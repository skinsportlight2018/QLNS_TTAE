...
<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
	<div class="container">
		<h1 class="h3 mb-0 custom-heading">Danh sách chuyên môn</h1>
	</div>
</nav>
						...
						?>
								<tr>
									<form method="post" action="index.php?action=capnhat">
										...
										<td>
											<input class="form-control" name="machuyenmon" type="text" value="<?php echo $cm["machuyenmon"]; ?>">
										</td>
										<td><input class="form-control" name="tenchuyenmon" type="text" value="<?php echo $cm["tenchuyenmon"]; ?>"></td>
							...
							?>
								<tr>
									<td><?php echo $cm["id"]; ?></td>
									<td ondblclick="this.setAttribute('contenteditable', 'true')"><?php echo $cm["machuyenmon"]; ?></td>
									<td ondblclick="this.setAttribute('contenteditable', 'true')"><?php echo $cm["tenchuyenmon"]; ?></td>
									<td><a href="index.php?action=sua&id=<?php echo $cm["id"]; ?>" class="btn btn-warning"><i class="bi bi-pencil-square"></i></a></td>
									<td><a href="javascript:void(0);" onclick="showDeleteMessage(<?php echo $cm['id']; ?>);" class="btn btn-danger"><i class="bi bi-trash-fill"></i></a></td>
								</tr>
						<?php
							}
						endforeach;
...
<script>
	function themHangMoi() {
		...
		var rowCount = <?php echo count($chuyenmon); ?>;
	...
	function luuHangMoi(row) {
		var maChuyenMon = row.cells[1].innerHTML;
		var tenChuyenMon = row.cells[2].innerHTML;
		...
		xhr.send("tenchuyenmon=" + encodeURIComponent(tenChuyenMon) + "&machuyenmon=" + encodeURIComponent(maChuyenMon));
	}
	...
	function saveRow(row) {
		...
		var maChuyenMon = row.cells[1].innerHTML; // Lấy mã chuyên môn từ ô đầu tiên của hàng
		var tenChuyenMon = row.cells[2].innerHTML; // Lấy tên chuyên môn từ ô thứ hai của hàng
		...
		xhr.send("id=" + encodeURIComponent(id) + "&tenchuyenmon=" + encodeURIComponent(tenChuyenMon) + "&machuyenmon=" + encodeURIComponent(maChuyenMon));
	}
</script>
