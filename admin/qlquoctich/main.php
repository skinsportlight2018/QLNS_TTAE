...
<nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
	<div class="container">
		<h1 class="h3 mb-0 custom-heading">Danh sách quốc tịch</h1>
	</div>
</nav>
						...
						?>
								<tr>
									<form method="post" action="index.php?action=capnhat">
										...
										<input type="hidden" name="id" value="<?php echo $qt["id"]; ?>">

										<td><input class="form-control" name="tenquoctich" type="text" value="<?php echo $qt["tenquoctich"]; ?>"></td>
							...
							?>
								<tr>
									<td><?php echo $qt["id"]; ?></td>
									<td ondblclick="this.setAttribute('contenteditable', 'true')"><?php echo $qt["tenquoctich"]; ?></td>
									<td><a href="index.php?action=sua&id=<?php echo $qt["id"]; ?>" class="btn btn-warning"><i class="bi bi-pencil-square"></i></a></td>
									<td><a href="javascript:void(0);" onclick="showDeleteMessage(<?php echo $qt['id']; ?>);" class="btn btn-danger"><i class="bi bi-trash-fill"></i></a></td>
								</tr>
						<?php
							}
						endforeach;
...
<script>
	function themHangMoi() {
		...
		var rowCount = <?php echo count($quoctich); ?>;
	...
	function luuHangMoi(row) {
		var tenQuocTich = row.cells[1].innerHTML;
		...
		xhr.send("tenquoctich=" + encodeURIComponent(tenQuocTich));
	}
	...
	function saveRow(row) {
		...
		var tenQuocTich = row.cells[1].innerHTML; // Lấy tên chuyên môn từ ô thứ hai của hàng
		...
		xhr.send("id=" + encodeURIComponent(id) + "&tenquoctich=" + encodeURIComponent(tenQuocTich));
	}
</script>
